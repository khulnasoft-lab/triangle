...
```powershell
## Import
Import-PfxCertificate -FilePath "DistroLauncher-Appx\DistroLauncher-Appx_TemporaryKey.pfx" -CertStoreLocation cert:\CurrentUser\My -Password (ConvertTo-SecureString -String "selfsignedcertpass" -AsPlainText -Force)
````

```console
## Create
openssl req -x509 -newkey rsa:2048 -keyout private.key -out certificate.crt -sha256 -days 3650 -nodes -subj "/CN=45FBC5C5-8DE5-4E85-AE6A-377002DA7605"

## Export
openssl pkcs12 -export -out DistroLauncher-Appx_TemporaryKey.pfx -inkey private.key -in certificate.crt -passout pass:

## View
openssl pkcs12 -in DistroLauncher-Appx_TemporaryKey.pfx -passin pass: -nokeys 2>/dev/null | awk -F ':' '/localKeyID/ {gsub(/ /,""); print $2}'
```
-->

<!--
- Code does work with Visual Studio 2022 (need to bump SDK/WindowsTargetPlatformVersion: `10.0.22000.0` & PlatformToolset: `v143`)
- Visual Studio 2019 is due to GitLab runners already have it pre-installed saving cycles/minutes
