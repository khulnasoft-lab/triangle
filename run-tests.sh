...
wait_for_triangle() {
    local image=$1 i web_up=no triangle_up=no
    ...
    for ((i = 0; i < 30; i++)); do
        info "$image: waiting for triangle to be ready ..."
...
wait_for_triangle_dce() {
    local image=$1-app i web_up=no triangle_up=no
...
sanity_check_image() {
    ...
    if [[ $2 == docker ]]; then
        ...
        if wait_for_triangle "$image"; then
            info "$image: OK !"
            result=ok
        else
        ...
        export PORT=$port
        export IMAGE=$1
        docker compose up -d triangle
        if wait_for_triangle_dce "$image"; then
            info "$image-app: OK !"
            result=ok
        else
